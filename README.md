...
- 已完成阶段 2 / 3 的 Sub2API 可用实现：
  - Go 路由 / handler / service / helper / test 节点抽取
  - handler -> service -> helper 的派生调用节点
  - 首批确定性规则引擎
- 已完成阶段 4 / 5 的 Sub2API 优先技术栈接入：
  - YAML / JSON 配置扫描
  - Markdown 台账 / 决策文档扫描
  - SQL migration 扫描
  - TS / Vue 页面、路由、导航、API、store 证据扫描
  - Markdown / JSON / bd draft 报告输出
...
- 可选 baseline 校验
- Go / Config / Markdown / SQL / TS/Vue 节点抽取
- 统一 IR 组装
- 决策文件加载
- 首批确定性规则执行
- 结构性 coverage finding 生成
- Markdown / JSON / bd draft 报告输出
...
- `语言适配器` 当前已覆盖 Go、YAML/JSON、Markdown、SQL、TS/Vue
- `规则引擎` 当前已落地首批确定性规则，并补充结构性 coverage finding
- `报告输出` 已支持 Markdown / JSON / bd draft
